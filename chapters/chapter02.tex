...
To determine the accuracy of the estimator, the dynamics of the system in the set $\{\bm{x}=\dot{V}(\bm{x})\}$ are analyzed. The staring equations of motion in \Cref{equation:EomWithTau} are premultiplied by $\bm{P}$ and $\bm{\omega}_p=\bm{0}$ is enforced which leads to
\begin{equation}
...
\begin{equation}
    ...
    \end{bmatrix}
\end{equation}
which explicitly written out become
\begin{subequations}
    \begin{align*}
