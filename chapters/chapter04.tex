...
\begin{equation}
 \bm{z} = m_s\bm{r}_0^{\times}\bm{g} - \bm{PK}_i\int\text{sign}(\eta_e)\bm{\epsilon}_e\,dt
\end{equation}
and for the controller to successfully minimize the horizontal imbalance, this value must tend towards zero. \Cref{fig:PID_sim_integral_error} shows the projected integral error of the controller in simulation. It is clear the integral action does indeed learn and compensate for the initial gravitional torque after about 40 seconds, further validating this approach. 
...
\toprule
\textbf{Method} & \textbf{Mean Torque [N·m]} & \textbf{max ($\Delta$KE) [J]} \\
\midrule
