...
Here, a rough estimate is obtained for the length of tests that may be run on the SADS after using the hybrid PID balancing method. The test considered is a simple repoint and hold maneuver, where the simulator attitude at steady-state is approximately constant. It will be assumed the average torque due to gravity is \SI{8.66e-4}{\newton\meter} and that the torque acts primarily in the horizontal plane of the body frame.  The representative disturbance torque is then $\bm{T}_{g,\text{avg}} = [8.66,\ 0,\ 0]^T\times\SI{e-4}{\newton\meter}$ which acts as a constant disturbance at steady-state. The simulator must generate an equal and opposite torque using the reaction wheel pyramid to hold its attitude. The change in angular momentum for each wheel is calculated as
\begin{equation}
...
\end{equation}
where $h_{0}$ is the angular momentum of the wheel after the transient slew. This yields a value of $t_{\text{sat}} \approx \SI{15}{\min}$. This is more than adequate for demonstrating repointing capability with the SADS, and opens up the possibility for more complex tracking maneuvers to be tested.
\iffalse
SIM RESULTS ------------------------------
Results PID:
horizontal: 2.5e-5
