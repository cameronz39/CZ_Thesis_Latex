...
By propgating a full distribution of representatives states through the nonlinear process, the effects of nonlinearity are better captured in the filter. The predicted mean and covariance are computed soley from the set of propgated sigma points. 
\begin{equation}
...
A simulation is developed in parallel to the SADS hardware to help verify design decisions and further validate results. Compenents are implemented in the simulation that model a variety of physical limitations the balancing performance.

\subsection{Plant Model}

The plant model solves the below differential equation

\begin{equation}
    \dot{\bm{\omega}} = \bm{J}^{-1}(-\bm{\omega}^\times \bm{J\omega} 
    - \bm{\omega}^\times\bm{h}_c + m_s\bm{r}_0^{\times}\bm{g} + m_s\Delta\bm{r}^{\times}\bm{g}
    - \dot{\bm{h}}_c)
\end{equation}
